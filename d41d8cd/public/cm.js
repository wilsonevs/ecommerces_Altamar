...
(function($){
	...
	$.fn.cmSetRecord = function(r){
		console.log("cmSetRecord",r);
		var selector=[
			"input[name]",
			"textarea[name]",
			"select[name]"
		...
		this.find(selector.join(",")).each(function(){
			...
			if( type=="radio" || type=="checkbox" ){
				alert("cmSetRecord Unimplemented radio and checkbox");
				return;
			}
	...
	*/
	...
	$.cmDialogConfirm=function(opts){
		opts = $.extend({
			selector:".cmdialog-confirm",
			title:"Información",
			message:"",
			onConfirm:function(){},
			onCancel:function(){}
