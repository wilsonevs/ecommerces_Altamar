<?php
namespace Cm;
require_once __DIR__."/fpdf.inc.php";


class JNcReport extends FPDF {
	private $doc = null;
	private $op;
	...
	public function drawLabel($x,$y,$el,$content){
		...
		//printWhen="$D{copia_dian}==false"
		/*
		$printWhen=$el->getAttribute("printWhen");
		if( $printWhen ){
		}
		*/
		...
		$this->setFont('HelveticaNeue',$fontWeight,(integer)$fontSize);
	...
	public function drawImage($x,$y,$el){
		...
		$imagePath=$this->resourcePath()."/{$image}";
	...
	public function drawBarcode($x,$y,$el,$content){
		...
		//$this->Code128($x, $y, "00011234567", 100, 100) ;
		$this->Code128($x, $y, $content, $width, $height) ;
		
		return;
	}
	
	public function processSection($x,$y,$section,$dsr=array(),$op=array()){
		$op["checkAddPage"]=isset($op["checkAddPage"]) ? $op["checkAddPage"]:true;
		
		//trRec for printWhen expression
		$trRec=array();
		foreach($dsr as $aux0=>$aux1){
			$field='$D{'.$aux0.'}';
			
			if( is_bool($aux1) ){
				$trRec[$field] = $aux1 ?"true":"false";
				continue;
			}
			
			$trRec[$field]=(string) $aux1?:"0"; //fixme
		}
		...
		foreach( $section->getElementsByTagName("*") as $el){
			...
			if( !empty($printWhen) ){
				...
				$expression=strtr($printWhen,$trRec);
				if( $expression!=$printWhen ){
					
					//error_log($printWhen);
					//error_log($expression);

					//fixme
					$mustPrint = eval('return '.$expression.';');
					//error_log("result=".($mustPrint?"true":"false"));
					if(!$mustPrint) continue;
				}
			}
	...
	public function processPageFooter(){
		...
		$this->processSection($x,$y,$section,array(),array("checkAddPage"=>false));
		$this->setY(  $y+$sectionHeight );
	}
	...
	public function processDetail($x,$y,$detail){
		...
		for($i=0;$i<$this->m_ds[$dsId]["query"]->size();$i++){
			...
			if( !$groupHeaderPainted && $groupHeader && $i==0 ){
				...
				$y=$this->processSection($x,$y,$groupHeader,$r);
				$y +=$groupHeaderHeight;
			}
			
			
			$y=$this->processSection( $x,$y,$items,$r);
			...
			if( !$groupHeaderPainted && $groupHeader && $groupExpValue != $this->fieldValue("ds",$groupExp,true) ){
				...
				if( $groupFooter ){
					...
					$y=$this->processSection($x,$y,$groupFooter,$r);
					$y+= $groupFooterHeight;
				}
	...
	/*
	public function currentDsRecord(){
	...
	*/
	...
	public function checkAddPage($y,$sectionHeight){
		$op=&$this->options;
		
		//print_r($op);exit;
		
		//$m="y={$y} m_pageFooterHeight={$this->m_pageFooterHeight} CurPageSize={$this->CurPageSize[1]} bottommargin={$op["bottommargin"]}";
		//error_log($m);
		/*if( $sectionHeight > $this->CurPageSize[1]-$op["topmargin"]-$op["bottommargin"] ){
			return false;
		}*/
		
		if( ($y+$sectionHeight+$this->m_pageFooterHeight) >= ( $this->CurPageSize[1]-$op["bottommargin"] ) ){
			$this->addPage();
			return true;
		}
	...
	public function runReport(&$db,$xmlfile,$p=array(),$orientation='P', $unit='mm', $size='Letter') {
		...
		$p["margins"] = isset($p["margins"]) ? $p["margins"]:"10,10,10,10";
		...
		$this->AddFont('HelveticaNeue','','HelveticaNeue.ttf',true);
		$this->AddFont('HelveticaNeue','B','HelveticaNeue-Bold.ttf',true);
		
		//$this->AddFont('Arial','','Arial Unicode.ttf',true);
