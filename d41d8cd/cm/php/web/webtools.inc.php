...
class WebTools {
	...
	*/
	...
	public static function pager(&$page,$params=[]){
		...
		if( $page->pageCount<=1 ){
			$html[]='<li class="current"><a>1</a></li>';
			$html[]='</ul>';
			return implode("",$html);
		}
		...
		if(false){
			...
			if( $currPage < 6 ){
				for($i=2; $i<7 &&$i<$page->pageCount;$i++){
					$pages[]=$i;
				}
			}
			elseif( $currPage > $page->pageCount-4 ){
				$i = $page->pageCount-5 >=0 ? $page->pageCount -5:1;
				...
				$min=$currPage-2 > 1 ? $currPage-2 : 2;
				$max=$currPage+2 < $page->pageCount ? $currPage+2: $page->pageCount;
			...
			if( count($pages) > 0 && $pages[0]!=2 ){
				array_unshift($pages,"...");
			}
			array_unshift($pages,1);

			if( end($pages)!= $page->pageCount-1 ){
				$pages[]="...";
			}
			...
			foreach($pages as $i){
				...
				if( $i=="..."  ){
					$html[]='<li class="unavailable"><a href="">…</a></li>';
					continue;
				}
	...
	public static function optimizeCss($buffer){
		// Remove comments
	...
	public static function bundleCss($files){

	}

	public static function optimizeJavascript(){
	}
